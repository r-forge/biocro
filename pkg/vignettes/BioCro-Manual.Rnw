...
<<c4photo-aq-aci, warning=FALSE, fig.height=3.8>>=
pr <- seq(0,2000)
...
<<c4photo-stress-gs, fig.height=4.5>>=
sws <- seq(0,1,0.01)
...
<<canopy-stress-graphs,echo=FALSE, fig.width=5, fig.height=4.5>>=
...
for further manipulation. This code works fine on my desktop and two
laptops but it fails in the R-forge build. Trying to find out why.
...
into. An example of a simulation using 7 layers and a soil depth of 1.5m.
