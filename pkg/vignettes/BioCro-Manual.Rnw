...
\begin{abstract}
  ...
  provides a nice environment for optimization and plotting.
\end{abstract}
...
\begin{itemize}
  \item Use values from the literature
    \item Optimize parameter values based on observed data
      ...
      Using values from the literature can work well most of the time, but the source
      of the values should be carefully assessed. For example, an important
      consideration is whether the values were reported at the same reference
      temperature. In BioCro these values are assumed to be at 25 $C$. Another
      consideration is that different models might have different parameterization of
      photosynthesis and what might work well in a different model might not work well
      in BioCro. 
      
      For optimization of these parameters based on observed data BioCro offers the
      ability to optimize these three parameters using different techniques. The first
      one is based on the \code{optim} function in R which will minimize the residual
      sum of squares (obs - sim). The first column has the curve ID, the second one
      the treatment which in this case is either Miscanthus (mxg) or switchgrass
      (swg). The third column is CO$_2$ assimilation, then photosynthetic active
      radiation in (PARin), temperature (Tleaf) and relative humidity (RH\_S).
...
fit <- nls(A ~ c4photo2(PARi, Tleaf, RH_S, vmax, alpha),
                       start=list(vmax=39, alpha=0.04), data = curve1)
@ 
...
\subsection{Effect of stress on diurnal transpiration}
...
\subsection{Calculation of EvapoTranspiration}

As in the CanA function transpiration can be calculated using
Priestly, Penman or Penman-Monteith.

<<BioGro-ET, echo=FALSE>>=
data(cmi05)
res.prs <- BioGro(cmi05, 
                  canopyControl = canopyParms(eteq="Priestly"))
res.pnm <- BioGro(cmi05, 
                  canopyControl = canopyParms(eteq="Penman"))
res.pnm.m <- BioGro(cmi05, 
                    canopyControl = 
...
xyplot(et.prs + et.pnm + et.pnm.m ~ res.prs$ThermalT, type='l',
       ...
       key=simpleKey(c("Priestly","Penman","Penman-Monteith"),
           lines=TRUE, points=FALSE))


@ 

For Priestly and Penman the calculation of stomatal conductance does
not affect the simulation, but for Penman-Monteith it does.

<<BioGro-ET-b1-12, echo=FALSE>>=
photoP <- photoParms(b1=12)
soilP <- soilParms(soilDepth=1.5)
res.pnm.m.b1.12 <- BioGro(cmi05, 
                          photoControl = photoP,
                          soilControl = soilP)
...
xyplot(et.prs + et.pnm + et.pnm.m + et.pnm.m.b1.12 ~ res.prs$ThermalT, 
       ...
       key=simpleKey(c("Priestly","Penman","PM b1=3",
           "PM b1=12"), lines=TRUE, points=FALSE))
@ 

Penman-Moneith does not quite reach Priestly because there is
stress. It is possible to perform simulations assuming that there is
no stress but this will likely lead to unrealistic results because the
crop is not limited at all by availability of soil
moisture. Transpiration in the no-stress scenario is very high.

<<BioGro-ET-no-stress, echo=FALSE>>=
photoP <- photoParms(b1=12)
soilP <- soilParms(wsFun='none')
res.pnm.m.ns <- BioGro(cmi05, 
                          photoControl = photoP,
                          soilControl = soilP)
...
\subsection{Soil information and parameters}
...
Another important information is the soil depth. Typically crops have
access to anywhere from 1 to 2.5 m of soil through their soil
...
res <- MaizeGro(cmi05, plant.day=110, emerge.day=117, 
