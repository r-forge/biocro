/*
 *  BioCro/src/BioCro.c by Fernando Ezequiel Miguez  Copyright (C) 2007-2014
 *
	...
	for(i=0;i<vecsize;i++)
	{
		...
		if(soillayers > 1){
			soilMLS = soilML(*(pt_precip+i), CanopyT, &cwsVec[0], soilDepth, REAL(SOILDEPTHS), FieldC, WiltP,
					 phi1, phi2, smthresh, soTexS, wsFun, INTEGER(SOILLAYERS)[0], Root, 
...
/* Populating the dbp coefficients */
		...
		BioGro(lati,INTEGER(DOY),INTEGER(HR),REAL(SOLAR),REAL(TEMP),REAL(RH),
		       ...
		       REAL(SECS), REAL(NCOEFS)[0], REAL(NCOEFS)[1], REAL(NCOEFS)[2], INTEGER(LNFUN)[0]);
	    ...
	    double cws[], int hydrDist, double secs[], double kpLN, double lnb0, double lnb1, int lnfun )
{
	...
	const double smthresh = soilcoefs[9];
	...
	for(i=0;i<vecsize;i++)
	{
		...
		if(soilLayers > 1){
			...
			soilEvap = SoilEvapo(LAI, 0.68, temp[i], solar[i], waterCont, FieldC, WiltP, windspeed[i], rh[i], secs[1], soilType);
			TotEvap = soilEvap + CanopyT;
			WaterS = watstr(precip[i],TotEvap,waterCont,soilDepth,FieldC,WiltP,phi1,phi2, smthresh, soilType, wsFun);   
			waterCont = WaterS.awc;
			StomWS = WaterS.rcoefPhoto ;
			LeafWS = WaterS.rcoefSpleaf;
